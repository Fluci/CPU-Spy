...
public final class FSString : CustomStringConvertible, CustomDebugStringConvertible, StringLiteralConvertible {
    ...
    private init(aStr : UnsafeMutablePointer<[CChar]>, aRfc : UnsafeMutablePointer<Int>, aRangeStart: Int, aLength: Int){
        str = aStr;
    ...
    */
    public func findNext(needle : FSString, start: Int = 0) -> Int {
        ...
        for(i = start; i < length; ++i){
            ...
            // self-reminder: this is not possible, since the needle could be in the just searched string
            // i = i + j;
        }
        return i;
    }
